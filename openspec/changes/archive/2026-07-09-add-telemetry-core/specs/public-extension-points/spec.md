...
The package SHALL document and test the public contracts that callers can implement or subclass to extend retry and resilience behavior without changing package internals, including telemetry listeners and severity providers.
...
#### Scenario: Caller implements telemetry listener
- **WHEN** a caller provides a custom telemetry listener
- **THEN** the package emits structured telemetry events to that listener without requiring package internals

### Requirement: Support callback-based custom extensions
The package SHALL provide callback-based custom factories for extension points where callers commonly need simple custom behavior, including telemetry listeners and severity providers.
...
Custom extension behavior SHALL receive retry metadata through `RetryContext<T>`, telemetry events, or a strategy context that exposes `RetryContext<T>` and SHALL be testable through public strategy APIs, `package:clock` time control, direct calculations, explicit callbacks, telemetry listeners, and event observers instead of runtime dependency injection.

#### Scenario: Custom behavior uses public testing paths
- **WHEN** tests configure clock zones, direct delay calculations, limiter leases, timeout operations, telemetry listeners, or event callbacks
...
The package SHALL keep custom extension support focused on retry execution, retry decisions, timing, fallback handling, circuit failure classification, hedging classification, rate limiter behavior, telemetry observation, and ordered pipeline wrapping.

#### Scenario: Resilience pattern is built into core
- **WHEN** a caller needs retry, timeout, fallback, circuit breaker, hedging, rate limiting, concurrency limiting, or telemetry observation
