...
A mainstream retry model exposes the same broad capability through `ShouldHandle`, `MaxRetryAttempts`, delay generation, jitter, and `OnRetry`. The goal for `retry_plus` is not to copy fixed option object or enum-based backoff API. The goal is to make the existing open Dart-style model expressive enough to cover those capabilities.
...
- Do not introduce fixed external API names or option objects.
- Do not add `BackoffType` or other fixed backoff enums.
- Do not implement non-retry resilience strategies such as timeout, fallback, circuit breaker, hedging, or rate limiting in this change.
...
This lets built-in combinators express common `ShouldHandle` and max-retry concepts, while custom callbacks can express domain-specific budgets.

### Decision: support `FutureOr` for decision, delay, and hook callbacks

A mainstream retry model supports asynchronous `ShouldHandle`, delay generation, and retry hooks. `retry_plus` will support the same capability through Dart `FutureOr` rather than separate sync/async APIs.
...
Generated delay may return `null`; callers can combine that with fallback delay behavior to model "use generated delay when valid, otherwise use calculated delay" semantics.
...
- **Result exhaustion semantics**: If `retryIf` returns false after retryable-looking results due to budget, the engine returns the final result to match fixed-option retry outcome behavior. Mitigation: keep give-up metadata for observation without turning the result into a failure.
...
4. Add delay combinators needed to cover mainstream retry delay behavior.
