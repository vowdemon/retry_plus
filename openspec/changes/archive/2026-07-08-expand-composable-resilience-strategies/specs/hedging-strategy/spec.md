...
The hedging strategy SHALL expose an asynchronous hook and pipeline events when hedged actions are scheduled and when action outcomes are observed.

#### Scenario: On hedge hook runs
- **WHEN** a hedged action is scheduled
- **THEN** the strategy invokes the configured hook with action index, delay, context, and currently observed outcomes

#### Scenario: Hedging event is emitted
- **WHEN** a hedged action is scheduled or selected
- **THEN** the pipeline observer receives a hedging event with public metadata
