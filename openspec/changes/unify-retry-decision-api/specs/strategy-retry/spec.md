...
### Requirement: Preserve Polly retry capability without Polly-style options
The package SHALL express Polly retry capabilities through open retry decision, delay, and hook extension points rather than a Polly-compatible options object or backoff enum.

#### Scenario: Polly max retry behavior is expressed through retryIf
- **WHEN** a caller needs Polly-style `MaxRetryAttempts`
- **THEN** the caller can express the same extra-retry budget through a retry decision combinator

#### Scenario: Polly backoff behavior is expressed through delay strategies
