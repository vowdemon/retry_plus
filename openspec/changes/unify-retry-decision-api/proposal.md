...
The current retry API splits continuation decisions between outcome predicates and stop strategies, which makes retry behavior harder to reason about and harder to extend toward Polly-level retry capability. Since this is still a development version, the retry API can be simplified around one decision model without preserving the older stop-oriented API.
